...
if m.get("version")!="1.0.10":
    raise SystemExit(f"Expected live base 1.0.10, got {m.get('version')}")
...
try {
    ...
    $marker='# BATCH PANEL STATE SAFE v1.0.11'
    if(-not $text.Contains($marker)){
        if(-not $text.Contains('# BATCH IMAGE LOADER SAFE FIX v1.0.10')){throw 'Could not find the v1.0.10 safe batch image loader.'}
        $start=$text.IndexOf('function Add-InlineBatchSplitter')
        $finish=$text.IndexOf('function Start-InlineBatchSplit',$start)
        if($start -lt 0 -or $finish -le $start){throw 'Could not isolate the current multi-wrap splitter function.'}

        $replacement=@'
# BATCH PANEL STATE SAFE v1.0.11
...
'@
        ...
        $oldFail='$failed.Add(([IO.Path]::GetFileName($p)+'': ''+$_.Exception.Message))'
        if($text.Contains($oldFail)){
            $newFail='$failed.Add(([IO.Path]::GetFileName($p)+'': ''+$_.Exception.Message+'' | ''+[string]$_.ScriptStackTrace))'
            $text=$text.Replace($oldFail,$newFail)
        }
    }
...
'@ | Set-Content -LiteralPath $targetVersion -Encoding UTF8
    ...
    try{
        [Windows.Forms.MessageBox]::Show(
            ('The Library could not install the batch panel state repair.'+[Environment]::NewLine+[Environment]::NewLine+$message+[Environment]::NewLine+[Environment]::NewLine+'The previous app version was restored.'),
            'The Library Update'
        )|Out-Null
    }catch{}
    Relaunch-App
...
name="payload-1.0.11-batch-panel-state-safe.txt"
...
validation={
    ...
    "payloadSha256":sha,
    "requirements":{
        "keepsSafeImageLoader":"# BATCH IMAGE LOADER SAFE FIX v1.0.10" in patcher,
        "replacesBatchSplitter":"# BATCH PANEL STATE SAFE v1.0.11" in patcher,
        "stateStoredOnControls":"$pic.Tag=$state" in patcher and "$save.Tag=$state" in patcher,
        "paintUsesSenderState":"Get-InlineBatchDisplayRect $sender $st.Bitmap" in patcher,
        "saveUsesControlState":"bitmap=$st.Bitmap" in patcher and "sourcePath=$st.Path" in patcher,
...
(TF/"batch-panel-state-safe-1.0.11-validation.json").write_text(json.dumps(validation,indent=2),encoding="utf-8")
vd=ROOT/".library-v111-validation"
