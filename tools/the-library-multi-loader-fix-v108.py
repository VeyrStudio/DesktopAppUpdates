...
if m.get("version")!="1.0.7":
    raise SystemExit(f"Expected live base 1.0.7, got {m.get('version')}")
...
try {
    ...
    if(-not $text.Contains($marker)){
        foreach($needed in @('function Add-LibraryRecord','function Copy-Or-Move-IntoVault','function Load-ImageUnlocked','$tabSplit')){
...
function Start-InlineBatchSplit {
    ...
    foreach($p in $paths){
        try{
            ...
            $n--
            $failed.Add(([IO.Path]::GetFileName($p)+': '+$_.Exception.Message))
        }
    }
    ...
    if($host.Controls.Count -eq 0){
        ...
        $host.Dispose()
        $script:InlineBatchHost=$null
        $detail=if($failed.Count -gt 0){[Environment]::NewLine+[Environment]::NewLine+(@($failed|Select-Object -First 5)-join [Environment]::NewLine)}else{''}
        [Windows.Forms.MessageBox]::Show(('None of those files could be opened as cover images.'+$detail),'Batch Split')|Out-Null
        return
    }

    if($failed.Count -gt 0){
        [Windows.Forms.MessageBox]::Show(("Some covers could not be opened: $($failed.Count)"+[Environment]::NewLine+[Environment]::NewLine+(@($failed|Select-Object -First 5)-join [Environment]::NewLine)),'Batch Split')|Out-Null
    }
}
...
name="payload-1.0.8-multi-loader-fix.txt"
(TF/name).write_bytes(raw)
sha=hashlib.sha256(raw).hexdigest()

validation={
    "version":VERSION,"baseVersion":"1.0.7","payload":name,"payloadSha256":sha,
    "requirements":{
        ...
        "usesExistingImageLoader":"Load-ImageUnlocked $path" in patcher,
        "showsPerFileLoadErrors":"Some covers could not be opened" in patcher,
...
(TF/"multi-loader-fix-1.0.8-validation.json").write_text(json.dumps(validation,indent=2),encoding="utf-8")
vd=ROOT/".library-v108-validation"
