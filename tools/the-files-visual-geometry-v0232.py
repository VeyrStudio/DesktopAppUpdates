...
page_new=r'''public class BookPagePanel : Panel {
    ...
    public BookPagePanel() { DoubleBuffered=true; AutoScroll=false; SetStyle(ControlStyles.AllPaintingInWmPaint|ControlStyles.OptimizedDoubleBuffer|ControlStyles.UserPaint,true); Resize += delegate { UpdateRegion(); }; }
    GraphicsPath PagePath(Rectangle r) {
        GraphicsPath p=new GraphicsPath(); int outer=18,inner=7;
        if(IsLeftPage){
            p.AddBezier(r.Left+outer,r.Top,r.Left+8,r.Top+2,r.Left+3,r.Top+14,r.Left+3,r.Top+30); p.AddLine(r.Left+3,r.Bottom-28);
            p.AddBezier(r.Left+3,r.Bottom-12,r.Left+9,r.Bottom-2,r.Left+outer,r.Bottom); p.AddLine(r.Right-inner,r.Bottom-4);
            p.AddBezier(r.Right-2,r.Bottom-18,r.Right-1,r.Top+18,r.Right-inner,r.Top+4); p.CloseFigure();
        } else {
            p.AddBezier(r.Left+inner,r.Top+4,r.Left+1,r.Top+18,r.Left+2,r.Bottom-18,r.Left+inner,r.Bottom-4); p.AddLine(r.Right-outer,r.Bottom);
            p.AddBezier(r.Right-9,r.Bottom-2,r.Right-3,r.Bottom-12,r.Right-3,r.Bottom-28); p.AddLine(r.Right-3,r.Top+30);
            p.AddBezier(r.Right-3,r.Top+14,r.Right-8,r.Top+2,r.Right-outer,r.Top); p.CloseFigure();
        }
        return p;
    }
    ...
    protected override void OnPaintBackground(PaintEventArgs e){
        e.Graphics.SmoothingMode=SmoothingMode.AntiAlias; Rectangle r=new Rectangle(0,0,Math.Max(1,Width-1),Math.Max(1,Height-1));
        using(GraphicsPath path=PagePath(r)){
            Color a=IsLeftPage?Color.FromArgb(244,226,188):Color.FromArgb(239,218,178); Color b=IsLeftPage?Color.FromArgb(220,188,140):Color.FromArgb(225,194,147);
            using(LinearGradientBrush paper=new LinearGradientBrush(r,a,b,IsLeftPage?5f:175f))e.Graphics.FillPath(paper,path); using(Pen edge=new Pen(Color.FromArgb(180,104,72,40),1.25f))e.Graphics.DrawPath(edge,path);
        }
        for(int i=0;i<70;i++){int x=13+Math.Abs((i*83+Width*7)%Math.Max(14,Width-26));int y=10+Math.Abs((i*47+Height*5)%Math.Max(12,Height-20));using(Brush dot=new SolidBrush(Color.FromArgb(8+(i%4)*3,92,61,34)))e.Graphics.FillEllipse(dot,x,y,(i%3)+1,(i%2)+1);}
        using(Pen ornament=new Pen(Color.FromArgb(105,118,78,42),1f)){int m=25,l=17;e.Graphics.DrawLine(ornament,m,m,m+l,m);e.Graphics.DrawLine(ornament,m,m,m,m+l);e.Graphics.DrawLine(ornament,Width-m,m,Width-m-l,m);e.Graphics.DrawLine(ornament,Width-m,m,Width-m,m+l);e.Graphics.DrawLine(ornament,m,Height-m,m+l,Height-m);e.Graphics.DrawLine(ornament,m,Height-m,m,Height-m-l);e.Graphics.DrawLine(ornament,Width-m,Height-m,Width-m-l,Height-m);e.Graphics.DrawLine(ornament,Width-m,Height-m,Width-m,Height-m-l);}
        Rectangle sh=IsLeftPage?new Rectangle(Math.Max(0,Width-44),0,44,Height):new Rectangle(0,0,44,Height); Color dark=Color.FromArgb(118,55,34,20),clear=Color.FromArgb(0,55,34,20);
        ...
        int sx=IsLeftPage?Width-8:7; using(Pen seam=new Pen(Color.FromArgb(120,77,48,28),1f))e.Graphics.DrawLine(seam,sx,14,sx,Height-14);
    }
}
...
tab_new=r'''public class BookTabButton : Button {
    ...
    protected override void OnPaint(PaintEventArgs e){
        e.Graphics.SmoothingMode=SmoothingMode.AntiAlias;Color t=ActiveTab?ControlPaint.Light(Tone,.22f):Tone;int notch=16;Point[] pts;
        ...
        using(Pen st=new Pen(Color.FromArgb(125,235,208,161),1f)){st.DashStyle=DashStyle.Dot;e.Graphics.DrawRectangle(st,stitch);} Rectangle tr=!Mirror?new Rectangle(8,2,Width-notch-12,Height-4):new Rectangle(notch+5,2,Width-notch-12,Height-4);
...
public class LeatherNavButton : Button {
    ...
    protected override void OnPaint(PaintEventArgs e){
        ...
        using(LinearGradientBrush br=new LinearGradientBrush(r,Color.FromArgb(36,23,17),Color.FromArgb(20,14,11),90f))e.Graphics.FillRectangle(br,r);
        using(Pen p=new Pen(Color.FromArgb(180,157,108,48),1.2f))e.Graphics.DrawRectangle(p,r);Rectangle inner=Rectangle.Inflate(r,-5,-5);using(Pen p2=new Pen(Color.FromArgb(72,213,177,101),1f))e.Graphics.DrawRectangle(p2,inner);
...
cur_label=re.search(r'\$currentNav=New-Object System\.Windows\.Forms\.Label',core)
if not cur_label: raise SystemExit('currentNav label creation not found')
core=core.replace("$currentNav=New-Object System.Windows.Forms.Label;", "$currentNav=New-Object System.Windows.Forms.Label; $currentNav.BackColor=[System.Drawing.Color]::FromArgb(42,27,19); $currentNav.ForeColor=[System.Drawing.Color]::FromArgb(232,201,145); $currentNav.BorderStyle='FixedSingle';",1)
...
if anim: core=core[:anim.start()]+"function Animate-BookPages([int]$Direction,[int]$Count,[bool]$Whole=$false) { return }"+core[anim.end():]
