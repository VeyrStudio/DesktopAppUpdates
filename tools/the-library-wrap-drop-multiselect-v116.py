    ...
    $Control.Add_DragDrop({
        ...
        if ($sender.Tag -eq "single") {
            ...
            elseif ($valid.Count -gt 1) {
                # Multiple dropped wraps go directly into the batch splitter.
...
'@
        ...
        if(-not $text.Contains($old)){throw 'Could not find the current one-file-only wrap drop handler.'}
