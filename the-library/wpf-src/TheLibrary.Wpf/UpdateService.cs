...
{
    private const string ManifestUrl = "https://raw.githubusercontent.com/VeyrStudio/DesktopAppUpdates/main/the-library/manifest.json";
    ...
    public async Task<UpdateCheckResult> CheckAsync()
    {
        ...
        if (manifest.AppId != "the-library") throw new InvalidDataException("The update feed belongs to another app.");
        ...
        if (remote <= local) return new UpdateCheckResult(null, $"The Library is up to date.\n\nInstalled version: {CurrentVersion}");
        return new UpdateCheckResult(manifest, $"A new update is available.\n\nInstalled: {CurrentVersion}\nAvailable: {manifest.Version}\n\n{manifest.Notes}");
    }
    ...
    public async Task DownloadAndStartAsync(UpdateManifest manifest, Window owner)
    {
        ...
        var work = Path.Combine(Path.GetTempPath(), "TheLibraryUpdate-" + Guid.NewGuid().ToString("N"));
        ...
        if (payload.AppId != "the-library" || payload.Version != manifest.Version) throw new InvalidDataException("The update payload does not match the manifest.");
        ...
        if (!File.Exists(Path.Combine(payloadDir, "TheLibrary.exe"))) throw new InvalidDataException("The update does not contain TheLibrary.exe.");

        var currentExe = Environment.ProcessPath ?? throw new InvalidOperationException("The Library could not locate its executable.");
        var updater = Path.Combine(work, "TheLibrary-Updater.exe");
    ...
    private static string ReadCurrentVersion(string appDir)
    {
        try
        {
            ...
            return json.RootElement.GetProperty("version").GetString() ?? "1.0.0";
        }
        catch { return "1.0.0"; }
    }
}
...
public sealed class UpdatePart { public string Url { get; set; } = ""; public string Sha256 { get; set; } = ""; }
...
public sealed class UpdateFile { public string Path { get; set; } = ""; public string Sha256 { get; set; } = ""; public string ContentBase64 { get; set; } = ""; }
